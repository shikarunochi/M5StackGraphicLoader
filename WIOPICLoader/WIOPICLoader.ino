...
expand_chain( int x, int y, int c)
{
  ...
  for ( ;; ) {
    ...
    if ( y_over == 0) {
      thunder[y<<16|x] = c;
      if (squareMode) {
        tft.drawPixel((x >> 1) +32, (y >> 1) - 8, rgb565);
      } else {
        tft.drawPixel((x * 2 / 3) -10, (y >> 1) - 8, rgb565);
      }
    }
  }
}
...
//X68000横2ドットをM5Stack1ドットとして、256 x 256 で正方形モードでの描画。512 / 2 = 256 なので、横32ドットずらして、画面真ん中に正方形で描く
  ...
  tft.setAddrWindow(0, dst_y, dst_width, 1);
  tft.pushColors(linebuf, dst_width, false);
}
