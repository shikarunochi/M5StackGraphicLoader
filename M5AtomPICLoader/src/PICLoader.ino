...
void randomDraw() {
  File picRoot;
  picRoot = SPIFFS.open(PIC_DIRECTORY);
  int fileCount = 0;
  while (1) {
...
expand_chain( int x, int y, int c)
{
  ...
  for ( ;; ) {
    ...
    if ( y_over == 0) {
      thunder[y<<16|x] = c;
      if (squareMode) {
        m5lcd.drawPixel((x >> 1) +32, (y >> 1) - 8, rgb565);
      } else {
        m5lcd.drawPixel((x * 2 / 3) -10, (y >> 1) - 8, rgb565);
      }
    }
  }
}
...
uint16_t swap565( uint16_t r, uint16_t g, uint16_t b) {
...
//X68000横2ドットをM5Stack1ドットとして、256 x 256 で正方形モードでの描画。512 / 2 = 256 なので、横32ドットずらして、画面真ん中に正方形で描く
  ...
  //GGGGGRRRRRBBBBBI から RGB565作成
        linebuf[dst_x] = makeRGB565(
         (getR(p11) + getR(p12) + getR(p21) + getR(p22)) / 4,
         (getG(p11) + getG(p12) + getG(p21) + getG(p22)) / 4 ,
         (getB(p11) + getB(p12) + getB(p21) + getB(p22)) / 4);
        m5lcd.drawPixel(dst_x,dst_y, linebuf[dst_x]);
      }
    }
  } else {
  ...
  //GGGGGRRRRRBBBBBI から RGB565作成
        if ((dst_x & 1) == 0) {  // 1:0.5で描画
          linebuf[dst_x] = makeRGB565(
           (getR(p11) + getR(p12) + (getR(p21) + getR(p22))*0.5f)  / 3,
           (getG(p11) + getG(p12) + (getG(p21) + getG(p22))*0.5f)  / 3,
           (getB(p11) + getB(p12) + (getB(p21) + getB(p22))*0.5f)  / 3);

        } else {                 // 0.5:1で描画
          linebuf[dst_x] = makeRGB565(
           ((getR(p11) + getR(p12))*0.5f + getR(p21) + getR(p22))  / 3,
           ((getG(p11) + getG(p12))*0.5f + getG(p21) + getG(p22))  / 3,
           ((getB(p11) + getB(p12))*0.5f + getB(p21) + getB(p22))  / 3);
  ...
  //m5lcd.setAddrWindow(0, dst_y, dst_width, 1);
  //m5lcd.pushColors(linebuf, dst_width, false);
